...
st.markdown(f"""
    ...
    .footer {{ position: fixed; bottom: 10px; left: 0; right: 0; text-align: center; color: #6c757d; font-size: 12px; font-weight: bold; }}
    </style>
...
def generar_id_y_procedencia(lote_txt):
    ...
    if lote_txt.isdigit():
        procedencia = "CDG"; id_gen = f"CDG-{lote_txt}-{timestamp}"
    elif "SF" in lote_txt:
        procedencia = "San Fernando"; id_gen = f"{lote_txt}-{timestamp}"
    elif "SE" in lote_txt:
        procedencia = "Santa Elena"; id_gen = f"{lote_txt}-{timestamp}"
    else:
        procedencia = "Otros"; id_gen = f"{lote_txt}-{timestamp}"
...
# --- 🟢 1. RECEPCIÓN (CON EDITOR ARREGLADO) ---
if choice == "🟢 Recepción":
    ...
    with t1:
        ...
        with st.form("form_ingreso", clear_on_submit=True):
            ...
            if st.form_submit_button("💾 GUARDAR REGISTRO"):
                if not lote_input or cant_h <= 0: st.error("⚠️ Lote y cantidad obligatorios.")
                else:
                    ...
                    conn.commit(); st.toast("Lote registrado", icon="📥"); st.success(f"ID: {id_u}"); st.balloons(); time.sleep(1); st.rerun()
    with t2:
        st.header("Editor de Lotes")
        lista_ids = pd.read_sql_query("SELECT id_unico FROM lotes", conn)
        id_edit = st.selectbox("Seleccione ID para corregir:", ["Seleccionar..."] + lista_ids['id_unico'].tolist())
        if id_edit != "Seleccionar...":
            df_edit = pd.read_sql_query(f"SELECT * FROM lotes WHERE id_unico='{id_edit}'", conn)
            if not df_edit.empty:
                datos = df_edit.iloc[0]
                with st.form("f_edit"):
                    ...
                    if st.form_submit_button("🔄 ACTUALIZAR DATOS"):
                        ...
                        conn.commit(); st.toast("Actualizado", icon="🔄"); st.success("Datos corregidos"); time.sleep(1); st.rerun()
...
# --- 🟡 INVENTARIO GLOBAL ---
    ...
    if not df.empty:
        ...
        if st.download_button("📥 DESCARGAR EXCEL", to_excel(df), "Inventario.xlsx"): st.toast("Descargando...", icon="📊")

# --- 📊 SEGUIMIENTO & DECISIONES (SEMAFORIZADO) ---
        ...
        with st.form("form_salida"):
            ...
            if st.form_submit_button("🚀 PROCESAR SALIDA"):
                ...
                if cant <= lote_info['saldo']:
                    ...
                    conn.commit(); st.toast(f"Salida: {id_s}", icon="📤"); st.success("✅ Salida Registrada"); st.balloons(); time.sleep(1.5); st.rerun()

# --- 🔍 5. FICHA DE TRAZABILIDAD (8 CUADROS) ---
