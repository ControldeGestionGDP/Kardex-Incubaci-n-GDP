...
st.set_page_config(page_title="IncubaTrack ERP v4.1", page_icon="🥚", layout="wide")

# --- SISTEMA DE BASE DE DATOS (REVISADO) ---
def init_db():
    conn = sqlite3.connect('incubacion_ultra_v4.db', check_same_thread=False)
    c = conn.cursor()
    # Tabla de Lotes con Procedencia y Planta
    ...
    # Historial que incluye la Planta para trazabilidad total
...
# ---------------------------------------------------------
# 🟢 1. RECEPCIÓN (CON PREFIJOS DINÁMICOS)
# ---------------------------------------------------------
if choice == "🟢 Recepción":
    ...
    with st.form("form_ingreso", clear_on_submit=True):
        ...
        genetica = c2.selectbox("Genética", ["Cobb 500", "Ross 308", "Hubbard"])
        edad_repro = c3.number_input("Edad de Reproductora (Sem)", min_value=1)
        ...
        if st.form_submit_button("REGISTRAR INGRESO"):
            ...
            try:
                c.execute("INSERT INTO lotes VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
                          (id_u, lote_input, proc, planta, granja, genetica, edad_repro, 
                ...
                st.error(f"Error: El lote ya fue registrado hoy o hay un conflicto: {e}")

# ---------------------------------------------------------
# 🟡 2. INVENTARIO GLOBAL (FILTROS Y CAJAS)
# ---------------------------------------------------------
    ...
    if not df.empty:
        ...
        st.dataframe(df[['id_unico', 'planta', 'procedencia', 'lote_nro', 'saldo', 'Cajas (360)', 'Días Almacén', 'fecha_postura', 'transportista']], use_container_width=True)
        st.download_button("📊 Exportar Excel", to_excel(df), "Inventario_Actual.xlsx")
    else:
        st.info("No hay stock disponible en ninguna cámara.")

# ---------------------------------------------------------
# 📊 3. SEGUIMIENTO & DECISIONES (INTELIGENCIA)
# ---------------------------------------------------------
    ...
    if not df_d.empty:
        ...
        def semaforo(row):
            ...
            if row['edad_repro'] > 60: return "🟠 REPRO VIEJA"
...
# ---------------------------------------------------------
# 🔍 4. TRAZABILIDAD (FICHA TÉCNICA)
# ---------------------------------------------------------
elif choice == "🔍 Trazabilidad por Lote":
    st.header("🧐 Ficha Técnica e Historial de Lote")
    ...
    if target != "Seleccionar...":
        ...
        c3.metric("Días en Planta", calcular_dias(info['fecha_llegada']))
        
        st.write(f"**Planta Original:** {info['planta']} | **Procedencia:** {info['procedencia']} | **Granja:** {info['granja']}")
        st.write(f"**Genética:** {info['linea_genetica']} | **Edad Repro:** {info['edad_repro']} semanas")
...
# ---------------------------------------------------------
    ...
    st.header("📋 Historial de Movimientos de Planta")
