...
if choice == "🟢 Recepción":
    ...
    with tab2:
        st.header("Editor de Datos Corregir")
...
# ---------------------------------------------------------
    ...
    if not lotes_activos.empty:
        with st.form("form_salida"):
            ...
            motivo_salida = st.selectbox("Motivo", ["Carga a Incubadora", "Venta", "Merma", "Ajuste"])
...
# ---------------------------------------------------------
    ...
    target = st.selectbox("Seleccione el Lote:", ["Seleccionar..."] + lotes_todos['id_unico'].tolist())
    ...
    if target != "Seleccionar...":
        ...
        with col_t2:
            st.subheader("🚚 Datos de Logística")
        ...
        st.divider()
        st.subheader("📜 Movimientos de este Lote")
